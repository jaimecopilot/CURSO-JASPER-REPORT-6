...
<div class="cover"><h1>Curso Profesional de JasperReports<br>6.20.0 Community</h1><h2>Módulo 4 — Parámetros y lógica</h2><div class="badge">{kind_es}</div><div class="author">AUTOR: JAIME GALLO</div><div class="tech">JasperReports Library 6.20.0 Community · Jaspersoft Studio 6.20.0 Community Edition · Java 8 · Maven · SQLite · CSV · XML · JSON · Proyecto EditorialReports</div></div>'''
    return '<!doctype html><html lang="es"><head><meta charset="utf-8"><title>M4</title><style>'+CSS+'</style></head><body>'+cover+str(soup)+'</body></html>'
...
def create_contact_sheets(pdf_path: Path, out_dir: Path, label: str):
    ...
    points=[]
    if label == "practica":
        needles=["Punto 4.1", "Parte B", "Parte C", "Punto 4.2", "Punto 4.3", "Punto 4.4", "Punto 4.5", "Punto 4.6", "Punto 4.7", "Reto resuelto", "Resultado esperado"]
    else:
        needles=["Punto 4.1", "Punto 4.2", "Punto 4.3", "Punto 4.4", "Punto 4.5", "Punto 4.6", "Punto 4.7", "Resumen rápido"]
    points=[0]
    for n in needles:
        p=find_page(doc,n)
...
def main():
    ...
    reports={}
    for kind,src,name in [("teoria",args.theory,"TEORIA_M4"),("practica",args.practice,"PRACTICA_M4")]:
    ...
    (out/"PRECHECK_M4.json").write_text(json.dumps(reports,ensure_ascii=False,indent=2),encoding="utf-8")
